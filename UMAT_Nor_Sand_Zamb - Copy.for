	...
	subroutine UMAT(strss,statev,ddsdde,sse,spd,scd, &
	 ...
	 if (DTIME==0.0d0) then
	  ...
	  Call Nor_Sand_Rate(noel, G, g_0, bk, nu, p_ref, nG, e_o, Gamma, lambda_c, M_tc, N, CHI_tc, H, &
		                 ...
		                 switch_smooth, N_S, S, e, psi, CHI_tce, p_i, pi_0, pi_max, M_i, SUM_rate, N_soft_i, switch_yield, &
	...
	!double precision
	double precision, intent(inout):: G, K, e, psi, CHI_tce, p_i, pi_0, pi_max, M_i
	...
	double precision:: M_i1, M_i2, p_i1, p_i2, e_1, e_2, psi_1, psi_2, CHIi_1, CHIi_2, CHI_tce1, CHI_tce2,&
					   G_1, G_2, K_1, K_2, e_c, dSP1(2), dSP2(2), pi_max_1, pi_max_2
	...
	double precision::  Theta, J3, J2, cos3Theta, Mtheta, km, M_itc !(km is the smoothness parameter)
	  ...
	  if (pi_max==0.0d0) then !Initialize pi_max required for undrained calculations
		  M_itc=M_tc*(1-CHIi*N*abs(psi)/M_tc)
		  pi_max=p*exp(-CHIi*psi/M_itc)
	  endif
	...
	!|________________________________________________________________________________________________________|
	  ...
	  if (FT <= FTOL) then !Elastic 
		  ...
		  M_itc=M_tc*(1-CHIi*N*abs(psi)/M_tc)
		  pi_max=pTrial*exp(-CHIi*psi/M_itc) !required for undrained calculations
	...
	!|______________________________________________________________________________________________________|
		  ...
		  call GetdSiganddSP (km, Locus, SigTrial, Epsp, dEpsSS, p_i1, pi_0, pi_max_1, S, M_i1, psi_1, CHI_tce1, M_tc, CHI_tc, CHIi_1&
	...
	!|______________________________________________________________________________________________________|
		  ...
		  call GetdSiganddSP (km, Locus, Sig1, Epsp1, dEpsSS, p_i2, pi_0, pi_max_2, S, M_i2, psi_2, CHI_tce2, M_tc, CHI_tc, CHIi_2&
	...
	!|______________________________________________________________________________________________________|			  
			  ...
			  pi_max=(pi_max_1+pi_max_2)/2.0d0
			  ...
			  !M_itc=M_tc*(1.0d0-CHIi*N*abs(psi)/M_tc) !Added for undrained and PSR softening
			  !pi_max=p*exp(-CHIi*psi/M_itc) !Added for undrained and PSR softening
			  ...
			  if (abs(F0)>FTOL) then !stress back to yield surface
				  call stressCorrection(km, ITER, FTOL, F0, dEpsSS, Sig, G_0, nu, p_ref, nG, p_i, pi_0, &
					  pi_max, S, M_i, M_tc, N, psi, CHIi, CHI_tc, CHI_tce, H, e_o, e, Gamma, Lambda_c, &
...
!/////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	!*********************************************************************************************************
	!_________________________________________________________________________________________________________
subroutine GetdSiganddSP(km, Locus, Sig, Epsp, dEps, p_i, pi_0, pi_max, S, M_i, psi, CHI_tce, M_tc, CHI_tc, CHIi, N, e_o, e &
	...
	double precision, intent(inout):: p_i, M_i, psi, CHI_tce, CHIi, e, G, K, dSP(2), pi_0, pi_max
	...
	dpimax=(p*exp(-CHIi*psi/M_itc))-pi_max
	eta_L=M_i*(1.0d0-CHIi*psi/M_itc)
	pi_max=p*exp(-CHIi*psi/M_itc) !Added for undrained and PSR softening
	Undrained_and_PSR_term=dFdSP(1)*S*q*dpimax/(p*eta_L) !consider creating a function to calculate this
	
	numerator=numerator+Undrained_and_PSR_term !Added term here
	...
	dSP=dSP+DSPErate
	dSP(1)=dSP(1)+(S*q*dpimax/(p*eta_L)) !Update p_i due to cap softening and PSR rotation (Need to update with Adel's work)
	!______________________________________________________________________________________________________
...
!	
!	
	subroutine stressCorrection(km, MAXIT, FTOL, F0, dEpsp, Sig, G_0, nu, p_ref, nG, p_i, pi_0, pi_max, S, M_i, M_tc, N, psi, CHIi, &
	...
	double precision, intent(in):: G_0, nu, p_ref, nG, M_tc, N, CHIi, CHI_tc, CHI_tce, H, km, S
	double precision, intent(inout):: p_i, M_i, e, psi, pi_max
	double precision, intent(in):: e_o, Gamma, lambda_e
	...
	M_itc=M_tc*(1.0d0-CHIi*N*abs(psi)/M_tc) !Added for undrained and PSR softening
	pi_maxn=p*exp(-CHIi*psi/M_itc) !Added for undrained and PSR softening
	dpimax=pi_maxn-pi_max
	eta_L=M_i*(1.0d0-CHIi*psi/M_itc)
	Undrained_and_PSR_term=dFdSP(1)*S*q*dpimax/(p*eta_L) !consider creating a function to calculate this
	
	numerator=F0+Undrained_and_PSR_term
	...
	!M_itc=M_tc*(1.0d0-CHIi*N*abs(psin)/M_tc) !Added for undrained and PSR softening
	!pi_maxn=p*exp(-CHIi*psin/M_itc) !Added for undrained and PSR softening
